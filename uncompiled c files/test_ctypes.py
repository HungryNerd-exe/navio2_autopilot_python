...
def estimator_loop(y,xh,servo):
    ...
    adc_lib,adc,imu_lib,imu,baro_lib,baro,ubl_lib,ubl = air.initialize_sensors()
    time.sleep(3)
    while True:
        initialEstTime = time.time()
        new_gps = air.read_sensor(y,adc_lib,adc,imu_lib,imu,baro_lib,baro,ubl_lib,ubl) # updates values in y
...
if __name__ == "__main__":
    ...
    y = multiprocessing.sharedctypes.RawArray(ctypes.c_float, 26) # imu, baro, gps, adc
    xh = multiprocessing.Array(ctypes.c_float, 12) # position, orientation, rates
    servo = multiprocessing.Array(ctypes.c_float, 13) # mode_flag, rcin, servo_out
    cmd = multiprocessing.Array(ctypes.c_float, 2) # psi_c, h_c
